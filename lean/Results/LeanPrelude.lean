...
expression.
-/
@[inline] def id' {α : Sort u} (a : α) : α := by canonical
...
-/
@[irreducible] def letFun' {α : Sort u} {β : α → Sort v} (v : α) (f : (x : α) → β x) : β v := by canonical
...
```
-/
abbrev inferInstance' {α : Sort u} [i : α] : α := by canonical
...
```
-/
abbrev inferInstanceAs' (α : Sort u) [i : α] : α := by canonical
...
-/
abbrev Unit' : Type := by canonical (count := 2) [PUnit]

/--
`Unit.unit : Unit` is the canonical element of the unit type.
It can also be written as `()`.
-/
@[match_pattern] abbrev Unit.unit' : Unit := by canonical
...
For more information: [Propositional Logic](https://lean-lang.org/theorem_proving_in_lean4/propositions_and_proofs.html#propositional-logic)
-/
def Not' (a : Prop) : Prop := by canonical (count := 30) [Pi, False]
...
-/
@[match_pattern] def rfl' {α : Sort u} {a : α} : Eq a a := by canonical

/-- `id x = x`, as a `@[simp]` lemma. -/
@[simp] theorem id_eq' (a : α) : Eq (id a) a := by canonical
...
structure If (a : Prop) (b : Prop) : Prop where
  h : a → b

theorem eq_of_heq' {α : Sort u} {a a' : α} (h : HEq a a') : Eq a a' :=
  -- by canonical +synth [If, cast]
  (HEq.rec (motive := fun {x_4 : Sort u} (x_5 : x_4) (x_6 : HEq a x_5) => ∀ (h : α = x_4), cast h a = x_5)
      (fun (x_4 : α = α) => rfl) h) rfl
...
theorem Or.resolve_left'  (h: Or a b) (na : Not a) : b := by canonical
theorem Or.resolve_right' (h: Or a b) (nb : Not b) : a := by canonical
theorem Or.neg_resolve_left'  (h : Or (Not a) b) (ha : a) : b := by canonical
theorem Or.neg_resolve_right' (h : Or a (Not b)) (nb : b) : a := by canonical
...
-/
@[reducible] def optParam' (α : Sort u) (default : α) : Sort u := by canonical (count := 2)
...
-/
@[reducible] def outParam' (α : Sort u) : Sort u := by canonical (count := 2)
...
for `α`.
-/
@[reducible] def semiOutParam' (α : Sort u) : Sort u := by canonical (count := 2)

set_option linter.unusedVariables.funArgs false in
/-- Auxiliary declaration used to implement named patterns like `x@h:p`. -/
@[reducible] def namedPattern' {α : Sort u} (x a : α) (h : Eq x a) : α := by canonical (count := 2)
...
axiom Bool.true_ne_false : Not (Eq true false)

theorem ne_false_of_eq_true' : {b : Bool} → Eq b true → Not (Eq b false)
  := by canonical [Bool.true_ne_false]
...
instance {α : Sort u} [Inhabited α] : Nonempty α := by canonical [Nonempty.intro]

/--
A variation on `Classical.choice` that uses typeclass inference to
infer the proof of `Nonempty α`.
-/
noncomputable def Classical.ofNonempty' {α : Sort u} [Nonempty α] : α := by canonical [Classical.choice]

instance {α : Sort u} {β : Sort v} [Nonempty β] : Nonempty (α → β) :=
  ⟨by canonical⟩

instance Pi.instNonempty' {α : Sort u} {β : α → Sort v} [inst: (a : α) → Nonempty (β a)] :
    Nonempty ((a : α) → β a) :=
    by canonical [Nonempty.intro, Classical.ofNonempty]

instance : Inhabited (Sort u) where
  default := by canonical (count := 2) [PUnit]

instance (α : Sort u) {β : Sort v} [Inhabited β] : Inhabited (α → β) where
  default := by canonical
...
theorem PLift.up_down' {α : Sort u} (b : PLift α) : Eq (up (down b)) b := sorry

/-- Bijection between `α` and `PLift α` -/
theorem PLift.down_up' {α : Sort u} (a : α) : Eq (down (up a)) a := by canonical
...
theorem ULift.up_down' {α : Type u} (b : ULift.{v} α) : Eq (up (down b)) b := sorry

/-- Bijection between `α` and `ULift.{v} α` -/
theorem ULift.down_up' {α : Type u} (a : α) : Eq (down (up.{v} a)) a := by canonical
...
abbrev DecidablePred' {α : Sort u} (r : α → Prop) : Sort _ :=
  sorry -- by canonical (count := 200) [Pi, Decidable]

/-- A decidable relation. See `Decidable`. -/
abbrev DecidableRel' {α : Sort u} {β : Sort v} (r : α → β → Prop) : Sort _ :=
  -- by canonical (count := 10000) [Pi, Decidable]
  sorry
...
-/
abbrev DecidableEq' (α : Sort u) : Sort _ :=
  sorry
  -- by canonical (count := 10000) [Pi, Eq, Decidable]
...
theorem of_decide_eq_true' [inst : Decidable p] : Eq (decide p) true → p := fun h =>
  by canonical [ne_true_of_eq_false, decide_eq_false]

theorem of_decide_eq_false' [inst : Decidable p] : Eq (decide p) false → Not p := fun h =>
  by canonical [ne_false_of_eq_true, decide_eq_true]
...
instance [DecidableEq α] : BEq α where
  beq a b := by canonical (count := 5) [decide]
...
@[macro_inline] instance {p q} [dp : Decidable p] [dq : Decidable q] : Decidable (And p q) :=
  sorry -- by canonical [Decidable, Decidable.isTrue, Decidable.isFalse] -- TODO
...
@[reducible] def GE.ge' {α : Type u} [LE α] (a b : α) : Prop := by canonical (count := 5) [LE.le]
/-- `a > b` is an abbreviation for `b < a`. -/
@[reducible] def GT.gt' {α : Type u} [LT α] (a b : α) : Prop := by canonical (count := 5) [LT.lt]

/-- Abbreviation for `DecidableRel (· < · : α → α → Prop)`. -/
abbrev DecidableLT' (α : Type u) [LT α] : Sort := sorry -- by canonical (count := 200) [DecidableRel, LT.lt]
/-- Abbreviation for `DecidableRel (· ≤ · : α → α → Prop)`. -/
abbrev DecidableLE' (α : Type u) [LE α] : Sort := sorry -- by canonical (count := 200) [DecidableRel, LE.le]
